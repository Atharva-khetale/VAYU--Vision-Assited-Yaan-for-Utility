...
async def speak_text(text):
    ...
    try:
        ...
        await asyncio.wait_for(asyncio.to_thread(_speak), timeout=10.0)
...
async def interactive_mode():
    ...
    while running and mode == 'b':
        ...
        question = await listen_for_speech(timeout=10)

        if question in ["timeout", "unknown", "error"]:
            await speak_text("I didn't hear anything. Please try again.")
            continue
...
async def run_system():
    ...
    # Start in navigation mode by default
    mode = 'a'
...
# Main async function
async def main():
    global get_frame  # Correctly declare global before use
